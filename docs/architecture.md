...
Recording version and release edition are separate domains. Only explicit user intent and recording-level evidence—recording or provider track titles, disambiguation/version fields, and validated performance evidence—may classify a recording as live, remix, acoustic, or another special version. Album and compilation titles are release context only: an appearance on a release whose name contains `Live` cannot turn an otherwise ordinary studio recording into a live performance. With no explicit special-version request, the intent defaults to studio/original.

Exact-track worker discovery does not depend on the model having found a source. It generates a small deterministic set of complete-credit, normalized-credit, and official-audio/video search hints; bounds results and unique probes across the whole job; deduplicates stable provider identities before probing; and stops when a strong safe candidate is validated. Collaboration punctuation is normalized for comparison and search without dropping a required collaborator or splitting structured artist names.
