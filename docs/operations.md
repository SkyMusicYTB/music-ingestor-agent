...
For exact natural-language tracks, the worker independently performs bounded curated-provider discovery after approval. It tries only a small deduplicated family of artist/title and official-audio/video queries and applies the existing probe, extractor, network, duration, and contradiction gates. A release or compilation name containing “Live” is not recording-version evidence. To retry a request affected by older incorrect inference, submit the same artist/title again (for example, `gabry ponte, kel - tarantella`) or retry its failed download after deploying this release; an explicitly requested `live version` remains live.
